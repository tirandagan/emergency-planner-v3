...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!response.ok) {
      ...
      console.error('[LLM Service] Job submission failed:', errorData);

      return NextResponse.json(
        { error: errorData.error || 'Failed to submit job' },
    ...
    console.error('[LLM Service] Job submission error:', error);
