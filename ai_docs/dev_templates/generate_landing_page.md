...
* `app/global.css` - Tailwind v3 theme implementation with tailwind.config.ts, CSS variables, and custom styles
...
* Review `app/globals.css` for existing theme variables (Tailwind v3 uses tailwind.config.ts)
...
**Folder Structure:**
- All landing page visuals go in `public/landing/`
- If the folder doesn't exist, tell user to create it in the project root via File Explorer or IDE
...
> Create `public/landing/` folder in the root project via your File Explorer or IDE if it doesn't exist.
...
2. Create `public/landing/` folder in the root project via your File Explorer or IDE
